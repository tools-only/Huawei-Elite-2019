...
def road_weight_decay(graph, t):
    ...
    for i in range(k-1, -1, -1): # 首先对前k个缺失进行填充
        ...
        for j in graph.road_value_t[t-i-1][0]:
            tem_road_value_t[j] = 0.9 * graph.road_value_t[t-i-1][0][j]
...
def cross_weight_decay(graph, t): 
    ...
    for i in range(k-1, -1, -1): # 首先对前k个缺失进行填充
        ...
        for j in graph.cross_value_t[t-i-1][0]:
            tem_cross_value_t[j] = 0.95 * graph.cross_value_t[t-i-1][0][j]
        graph.cross_value_t[t-i].append(tem_cross_value_t)

def load_traffic(graph, pre_cars, pre_path, pre, now):
    '''
    加载pre至当前时刻的预置车辆交通状态
    '''
    load_cars = pre_cars.loc[(pre_cars['planTime'] > pre)&(pre_cars['planTime'] <= now)] 
